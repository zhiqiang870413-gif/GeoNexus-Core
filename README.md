...
GeoNexus Core 是一款基於 Web GIS 技術的設施管理解決方案。採用 前後端分離 架構，後端基於 ASP.NET Core 8.0 並嚴格遵循 Service Layer (服務層) 設計模式與 JWT 安全驗證，前端則結合 Vue.js 與 Leaflet 提供直觀的地理空間操作介面。
...
雲端環境: 自動解析 DATABASE_URL 並連接 PostgreSQL。
